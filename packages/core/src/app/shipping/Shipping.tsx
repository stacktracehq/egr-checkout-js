...
class Shipping extends Component<ShippingProps & WithCheckoutShippingProps, ShippingState> {
  ...
  async componentDidMount(): Promise<void> {
    ...
    mapsApiScript.src =
      'https://maps.googleapis.com/maps/api/js?key=AIzaSyD_DJqzi5tcNvhZ2o-q-MsYLy0Lcw8VHn4&libraries=places';
  ...
  findStateByLatLng = (latLng: google.maps.LatLngLiteral): State | undefined => {
  ...
  filterSortFitmentCentresByDistance = (latLng: google.maps.LatLngLiteral): FitmentCentre[] => {
    const filteredFitmentCentres = reduce(
      fitmentCentres as FitmentCentre[],
      (acc: FitmentCentre[], fitmentCentre: FitmentCentre): FitmentCentre[] => {
  ...
  filterSortFitmentCentresByState = (state: State): FitmentCentre[] => {
    ...
    const reducedFitmentCentres = reduce(
      ...
      (acc: FitmentCentre[], fitmentCentre: FitmentCentre, _): FitmentCentre[] => {
        const distance = haversine(latLng, {
          lat: fitmentCentre.latitude,
          lng: fitmentCentre.longitude,
        });
  ...
  render(): ReactNode {
    ...
    const { isInitializing, isLoading } = this.state;
      ...
      return (
        <>
          <div className="fitment-message">
            Please select your preferred fitment centre location. Your items will be shipped to your
            nominated fitment provider.
          </div>
          <div>
          ...
          <div className="fitment-view-selector">
            <div
              className={`${this.state.showMap ? '' : 'selected'}`}
  ...
  private handleSingleShippingSubmit: (values: SingleShippingFormValues) => void = async ({
    ...
    const updatedOrderComment = authorityToLeave
      ? 'Authority to leave. ' + orderComment
      : orderComment;
